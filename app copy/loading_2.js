...
const Loading = () => {
  ...
  const checkLoginStatus = async () => {
    try {
      ...
      if (userInfoString) {
        // ユーザーが既にログインしている場合、遷移を行う
        setAnimationComplete(true);
        navigation.navigate('select'); // ログイン済みの場合の遷移先
      } else {
        // ユーザーが未ログインの場合、遷移を行う
        setAnimationComplete(true);
        navigation.navigate('login_toroku'); // 未ログインの場合の遷移先
      }
    } catch (error) {
      console.error('確認ログイン状態時にエラーが発生しました:', error);
      // エラーが発生した場合に適切な処理を追加
    }
  };
  ...
  useEffect(() => {
    ...
    firstAnimation_1.start(() => {
      secondAnimation_1.start(() => {
        // アニメーションが完了したらanimationCompleteをtrueに設定
        if (isMounted) {
          setTimeout(() => {
            checkLoginStatus();
          }, 3000);
